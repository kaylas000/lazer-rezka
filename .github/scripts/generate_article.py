...
def generate_article(topic, api_key):
    ...
    prompt = f"""Напиши экспертную SEO-статью для блога цеха лазерной резки в Голицыно на тему: "{topic['title']}"

ТЫ — ТЕХНОЛОГ ЦЕХА с 10-летним опытом. Пиши от первого лица («мы», «в нашем цехе», «я рекомендую»). Тон: экспертный, конкретный, с цифрами и фактами. Без воды и общих фраз.

ОБЯЗАТЕЛЬНЫЕ ФАКТЫ О ЦЕХЕ (используй органично в тексте):
- Волоконный лазер 3 кВт, точность позиционирования ±0.01 мм
- Режем: сталь до 20 мм, нержавейку до 12 мм, алюминий до 10 мм, медь/латунь до 6 мм
- Адрес: Голицыно, Заводской проспект, 34, Одинцовский городской округ, Московская область
- Работаем с 2020 года, 500+ заказов, доставка по Москве и МО
- Принимаем DXF/DWG, расчёт за 1 час

ТРЕБОВАНИЯ К КОНТЕНТУ:
- Объём: 800-1000 слов
- Формат: Markdown с таблицами, **жирным** выделением ключевых цифр и терминов
- Абзацы: 2-4 предложения
- Без эмодзи
- LSI-слова в тексте: допуски, раскрой, кромка, ЧПУ, контур, полилайн, квалитет, зона реза

ОБЯЗАТЕЛЬНАЯ СТРУКТУРА (строго по порядку):

1) Введение — 3-4 предложения без заголовка. Сразу ключевая фраза + гео (Москва, Голицыно, Московская область). Одно-два предложения с цифрами о цехе.

2) 3-4 раздела H2 с технической глубиной. Каждый раздел:
   - 2-3 абзаца с цифрами и фактами
   - Где уместно — таблицу (Markdown table) со сравнительными данными
   - **Жирным** выделяй ключевые параметры: **сталь до 20 мм**, **точность ±0.01 мм**, **3 кВт**

3) ## Практические советы — ровно 10 советов нумерованным списком. Каждый совет:
   - Начинается с глагола или ключевого действия
   - 2-3 предложения с конкретикой
   - Где уместно — внутренняя ссылка на услугу

4) ## Часто задаваемые вопросы — ровно 6 вопросов/ответов. Каждый:
   - **Вопрос** выделен жирным
   - Ответ: 2-4 предложения с цифрами и фактами
   - Вопросы содержат низкочастотные поисковые запросы

5) ## Заключение — 3-4 предложения. Подведение итогов + CTA-блок ровно такого формата:
...
- 3-5 внутренних ссылок: /services/metal/, /services/bending/, /services/powder-coating/, /services/welding/, /services/sandblasting/, /services/locksmith/, /calculator/, /contacts/
- 1-2 ссылки на другие статьи блога (формат: /blog/YYYY/MM/slug/)
- Гео-упоминания (Москва, Голицыно, Московская область) — минимум 3 раза
- В FAQ — низкочастотные поисковые запросы в вопросах

НЕ ДЕЛАЙ:
- Не выдумывай точные цифры скоростей и режимов если не уверен
- Не пиши «±0,01 мм» (правильно: «±0.01 мм»)
- Не используй эмодзи
- Не пиши короче 800 слов
- Не пропускай ни одного раздела структуры"""

    system = 'Ты — технолог цеха лазерной резки металла с 10-летним опытом. Пишешь экспертные статьи для блога с таблицами, цифрами и технической глубиной. Стиль: профессиональный, конкретный, от первого лица.'
    return call_worker(prompt, system=system, max_tokens=3000)
...
{{
  ...
  "description": "Мета-описание 140-160 символов, с цифрами и CTA. Заканчивается на 'Цех в Голицыно, Московская область.'",
  "slug": "{topic['slug']}",
  "category": "technical|materials|tips|application",
  "tags": ["тег1", "тег2", "тег3", "тег4"],
  "keywords": "лазерная резка, тема статьи, Москва, Голицыно, дополнительные ключи"
}}
...
def create_post_file(topic, content, metadata):
    ...
    front_matter = {
        ...
        'image': '/assets/images/blog/' + metadata['slug'] + '.webp',
